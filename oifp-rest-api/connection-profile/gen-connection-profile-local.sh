...
# This script uses the template oifp-connection-profile.json to generate a connection profile
...
cp oifp-connection-profile.yaml $REPODIR/tmp/connection-profile
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    sed -e "s|%REPODIR%|$CERTDIR|g" oifp-connection-profile.yaml > $REPODIR/tmp/connection-profile/oifp-connection-profile.yaml
else
    sed -i "s|%REPODIR%|$CERTDIR|g"  $REPODIR/tmp/connection-profile/oifp-connection-profile.yaml
fi 
