...
export class InvestorchartComponent implements OnInit, OnChanges {
  ...
  updateChart = (firstRun: boolean) => {
    ...
    this.investment = this.slices.map(slice => slice.investment);
    this.colourSlices = this.slices.map(slice => this.pieColours(slice.name));

    this.values = firstRun ? [0, 0, 0] : _.toArray(this.slices).map(slice => slice.investment);
  ...
  mouseover = (d, i) => {
    ...
    this.svg.append('text')
      ...
      .attr('class', 'label')
      .attr('fill', '#57a1c6')
      .text(this.labels[i] + ' contribution is $ ' + this.investment[i]);
