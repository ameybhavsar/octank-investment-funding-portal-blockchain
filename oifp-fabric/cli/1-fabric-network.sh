...
echo Executing command: aws managedblockchain create-network --region $REGION \
    ...
    --name "${NETWORKNAME}" \
    --description "OIFP Fabric network"
    ...
    --member-configuration "Name=\"${MEMBERNAME}\",Description=\"OIFP Fabric member\",FrameworkConfiguration={Fabric={AdminUsername=${ADMINUSER},AdminPassword=${ADMINPWD}}}"
